...
export default function HomePage() {
  ...
  // per-card phase tracking: 'idle' | 'download' | 'upload' | 'done' | 'error'
  const [netPhase, setNetPhase] = useState<string[]>(['idle','idle','idle','idle'])
  const [roomPhase, setRoomPhase] = useState<string[]>(['idle','idle'])
  ...
  /* ── network speed test (client-side) ── */
  const runNetSpeedTest = async (index: number) => {
    setNetPhase(p => { const a=[...p]; a[index]='download'; return a })
    try {
      const result = await runSpeedTest((phase) => {
        setNetPhase(p => { const a=[...p]; a[index]=phase; return a })
      })
      setNetworkTests(arr => {
        const a = [...arr]
        a[index] = { ...a[index], download: String(result.download), upload: String(result.upload) }
        return a
      })
      setNetPhase(p => { const a=[...p]; a[index]='done'; return a })
    } catch {
      setNetPhase(p => { const a=[...p]; a[index]='error'; return a })
    }
  }

  const runRoomSpeedTest = async (index: number) => {
    setRoomPhase(p => { const a=[...p]; a[index]='download'; return a })
    try {
      const result = await runSpeedTest((phase) => {
        setRoomPhase(p => { const a=[...p]; a[index]=phase; return a })
      })
      setRoomChecks(arr => {
        const a = [...arr]
        a[index] = { ...a[index], internetDown: String(result.download), internetUp: String(result.upload) }
        return a
      })
      setRoomPhase(p => { const a=[...p]; a[index]='done'; return a })
    } catch {
      setRoomPhase(p => { const a=[...p]; a[index]='error'; return a })
    }
  }
  ...
  const handleClear = () => {
    ...
    setNetPhase(['idle','idle','idle','idle'])
    setRoomPhase(['idle','idle'])
    setReportText('')
    setShowReport(false)
  ...
  /* ── phase label helper ── */
  const phaseBtn = (phase: string) => {
    if (phase === 'download') return { label: '↓ กำลังโหลด...', cls: styles.testBtnLoading }
    if (phase === 'upload')   return { label: '↑ กำลังส่ง...', cls: styles.testBtnLoading }
    if (phase === 'done')     return { label: '✓ Done', cls: styles.testBtnDone }
    if (phase === 'error')    return { label: '✕ Retry', cls: styles.testBtnError }
      ...
      <main className={styles.main}>
        ...
        {!showReport && (<>
          ...
          <div className={styles.infoNote}>
            <span>📡</span>
            <span>Speed test วัดจาก <strong>device นี้</strong> ผ่าน Cloudflare — ค่าที่ได้คือความเร็ว WiFi จริงของพื้นที่นั้น</span>
          </div>
            ...
            <div className={styles.sectionBody}>
              <div className={styles.netGrid}>
                {networkTests.map((n, i) => {
                  const btn = phaseBtn(netPhase[i])
                  const isTesting = netPhase[i] === 'download' || netPhase[i] === 'upload'
                  return (
                    <div key={n.location} className={styles.netCard}>
                      <div className={styles.netCardHead}>
                        <span className={styles.netLocLabel}>
                          {LOC_ICONS[n.location]} {LOCATION_LABELS[n.location]}
                        </span>
                        <button onClick={() => runNetSpeedTest(i)} disabled={isTesting}
                          className={`${styles.testBtn} ${btn.cls}`}>
                          {isTesting && <span className={styles.spinner}/>}
                          {btn.label}
                        </button>
                      </div>
                      {isTesting && (
            ...
            <div className={styles.sectionBody}>
              <div className={styles.grid3}>
                <div className={styles.field}>
                  ...
                  <input type="number" step="0.1" placeholder="23.5" value={serverRoom.tempIn}
                    onChange={e => setServerRoom({...serverRoom, tempIn: e.target.value})}/>
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>Temp Out (°C)</label>
                  <input type="number" step="0.1" placeholder="23.2" value={serverRoom.tempOut}
                    onChange={e => setServerRoom({...serverRoom, tempOut: e.target.value})}/>
                </div>
                <div className={styles.field}>
                  <label className={styles.label}>
                    Humidity (%)
                    {humidStatus && <span className={`${styles.humidBadge} ${humidStatus.cls}`}>{humidStatus.label}</span>}
                  </label>
                  <input type="number" step="1" min="0" max="100" placeholder="59" value={serverRoom.humidity}
              ...
              <div style={{marginTop:10}} className={styles.field}>
                ...
                <textarea placeholder="หมายเหตุ..." value={serverRoom.remark}
            ...
            <div className={styles.sectionBody}>
              <div className={styles.upsList}>
                    ...
                    <div className={styles.upsCardHead}>
                      ...
                      {upsChecks.length > 1 && (
                        <button className={styles.removeBtn}
                          onClick={() => setUpsChecks(upsChecks.filter((_,idx)=>idx!==i))}>✕</button>
                      )}
                    </div>
                ...
                <button className={styles.addBtn}
                  onClick={()=>setUpsChecks([...upsChecks,{building:'T1',backupMin:'',tempC:'',remark:''}])}>
                  + เพิ่ม UPS
                </button>
              </div>
            </div>
          ...
          <div className={styles.section}>
            ...
            <div className={styles.sectionBody}>
              <div className={styles.roomGrid}>
                {roomChecks.map((r, i) => {
                  const btn = phaseBtn(roomPhase[i])
                  const isTesting = roomPhase[i]==='download'||roomPhase[i]==='upload'
                  return (
                    <div key={i} className={styles.roomCard}>
                      ...
                      <div className={styles.roomCardBody}>
                        ...
                        <div className={styles.netRoomRow}>
                          <div className={styles.speedField}>
                            <span className={`${styles.miniLabel} ${styles.dlLabel}`}>↓ DL (Mbps)</span>
                            <input type="number" step="0.1" placeholder="—" value={r.internetDown}
                              onChange={e=>updateRoom(i,'internetDown',e.target.value)}/>
                          </div>
                          <div className={styles.speedField}>
                            <span className={`${styles.miniLabel} ${styles.ulLabel}`}>↑ UL (Mbps)</span>
                            <input type="number" step="0.1" placeholder="—" value={r.internetUp}
                              onChange={e=>updateRoom(i,'internetUp',e.target.value)}/>
                          </div>
                          <button onClick={()=>runRoomSpeedTest(i)} disabled={isTesting}
                            className={`${styles.testBtnSm} ${btn.cls}`}>
                            {isTesting?<span className={styles.spinner}/>: roomPhase[i]==='done'?'✓':'▶'}
                          </button>
                        </div>
                        {isTesting && (
                          <div className={styles.progressBar}>
        ...
        <div className={styles.modalOverlay} onClick={()=>setShowClearModal(false)}>
          <div className={styles.modal} onClick={e=>e.stopPropagation()}>
            ...
            <div className={styles.modalBtns}>
              <button className={styles.modalCancelBtn} onClick={()=>setShowClearModal(false)}>ยกเลิก</button>
              <button className={styles.modalConfirmBtn} onClick={handleClear}>ล้างข้อมูล</button>
            </div>
          </div>
        </div>
      )}
